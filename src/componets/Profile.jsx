...
function Profile() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("exisitingUser")) {
      const existingUserDetails = JSON.parse(sessionStorage.getItem("exisitingUser"))
      setUserDetails({
      ...
      <Collapse in={open}>
        <div className='row justify-content-center shadow rounded p-3' id="example-collapse-text">
          ...
          <div className="d-grid">
            <button onClick={handleUserProfile} className='btn btn-warning'>Updtae Profile</button>
          </div>
        </div>
      </Collapse>
