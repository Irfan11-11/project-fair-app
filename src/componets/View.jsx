...
function View() {
  ...
  const hendleDeleteProject = async (projectId) => {
    const token = sessionStorage.getItem("token")
    if (token) {
      const reqHeader = {
            ...
            userProjects?.map(project => (
              <div className="d-flex justify-content-between border p-2 rounded mb-3">
                ...
                <div className="icons d-flex">
                  ...
                  <button onClick={() => hendleDeleteProject(project?._id)} className="btn"><i className="fa-solid fa-trash text-danger"></i></button>
                </div>
              </div>
